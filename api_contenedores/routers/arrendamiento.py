...
def get_arrendamientos(db: Session = Depends(get_db)):
...
def get_proximos_vencer(dias: int = Query(7, ge=1, le=90, description="Días de anticipación para la alerta"), db: Session = Depends(get_db)):
    """
...
def get_arrendamiento(arrendamiento_id: int, db: Session = Depends(get_db)):
...
def create_arrendamiento(data: ArrendamientoCreate, db: Session = Depends(get_db)):
...
def update_arrendamiento(arrendamiento_id: int, data: ArrendamientoUpdate, db: Session = Depends(get_db)):
    a = db.query(Arrendamiento).filter(Arrendamiento.id_arrendamiento == arrendamiento_id).first()
    if not a:
        raise HTTPException(status_code=404, detail="Arrendamiento no encontrado")
...
def delete_arrendamiento(arrendamiento_id: int, db: Session = Depends(get_db)):
    a = db.query(Arrendamiento).filter(Arrendamiento.id_arrendamiento == arrendamiento_id).first()
    if not a:
        raise HTTPException(status_code=404, detail="Arrendamiento no encontrado")
    db.delete(a)
