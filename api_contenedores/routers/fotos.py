...
def get_fotos(
    ...
    return db.query(models.FotoContenedor).filter(
...
def upload_foto(
    ...
    nueva_foto = models.FotoContenedor(
...
def delete_foto(
    ...
    foto = db.query(models.FotoContenedor).filter(
        models.FotoContenedor.id_foto == foto_id
    ).first()
    if not foto:
